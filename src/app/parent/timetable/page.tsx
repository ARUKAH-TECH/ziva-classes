...
import { listMyChildrenSchedule } from "@/lib/actions/schedules";
...
export default async function ParentTimetablePage() {
  const schedule = await listMyChildrenSchedule();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1>Timetable</h1>
        <p className="mt-1 text-sm text-ink-500">Your children&apos;s recurring weekly class schedule.</p>
      </div>
        ...
        <CardContent>
          {byDay.length === 0 ? (
            <EmptyState
              ...
              description="Nothing scheduled for your children yet, or their subjects haven't been assigned."
            />
          ) : (
            <div className="space-y-6">
              {byDay.map((d) => (
