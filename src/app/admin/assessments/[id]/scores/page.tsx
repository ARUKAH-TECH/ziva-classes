...
export default async function AdminEnterScoresPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const [assessment, roster] = await Promise.all([getAssessmentContext(id), getAssessmentRoster(id)]);

  if (!assessment) notFound();

  return <EnterScores assessment={assessment} initialRoster={roster} backHref="/admin/assessments" />;
}
